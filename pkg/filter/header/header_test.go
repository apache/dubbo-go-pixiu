...
func TestHeader(t *testing.T) {
	...
	c := mock.GetMockHTTPContext(request, header, recovery.GetMock())
	c.API(api)
	c.Next()
	...
	c1 := mock.GetMockHTTPContext(request, header, recovery.GetMock())
	c1.API(api)
	c1.Next()
	assert.Equal(t, int8(len(c1.Filters)*2-1), c1.BaseContext.Index)

	request.Header.Set("filter", "test1")
	c2 := mock.GetMockHTTPContext(request, header, recovery.GetMock())
	c2.API(api)
	c2.Next()
