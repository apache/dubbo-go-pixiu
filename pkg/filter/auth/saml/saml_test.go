...
func TestDecode_ProtectedPath_NoSession(t *testing.T) {
	f := buildTestFilter(t)
	// Replace session provider with mock that returns error
	f.middleware.Session = &mockSessionProvider{err: errors.New("no session")}

	ctx, rec := newHttpContext("GET", "/api/users")
	result := f.Decode(ctx)

	assert.Equal(t, filter.Stop, result)
	// Should redirect to IdP (302) or return an auth-related response
