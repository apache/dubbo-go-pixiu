...
// three hookpoints: OnInitialize (optional pre-compute), Select (tools/list
...
type SelectionContext struct {
	...
	Headers    map[string]string // raw request headers (sensitive ones stripped)
	Claims     map[string]any    // JWT claims already validated by the auth/mcp filter
	Requested  string            // target tool name on tools/call
	UserPrompt string            // optional; used only by schema matching
...
type SelectionPlan struct {
	SessionID string
	ToolNames []string        // selected tool names, stable order
	Mode      string          // ModePassthrough | ModePolicy | ...
	Reasons   []DecisionTrace // per-candidate keep/drop reasons, for audit
	Version   string          // metadata snapshot version (registry + config hash)
	CreatedAt int64           // unix nano
	ExpiresAt int64           // 0 = session-lifetime valid
}
...
type DecisionTrace struct {
	Tool   string
	Kept   bool
	Stage  string // StagePolicy | StageWorkflow | StageSchema | StageProgressive
	Rule   string // matched rule / workflow name
	Detail string // short, non-PII explanation
}

// ToolSelector is the unified entry point invoked by the MCP server filter.
// Implementations may be passthrough, composite, or a test mock.
type ToolSelector interface {
	...
	// AuthorizeCall enforces the plan at tools/call time. A nil return allows
	// the call. When enforce_on_call is disabled the implementation may allow
	// calls even without a session plan.
	AuthorizeCall(ctx context.Context, sc SelectionContext) error

	// OnInitialize gives the selector a chance to pre-compute a base plan.
