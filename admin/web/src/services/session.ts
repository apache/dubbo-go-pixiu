...
export function readSession():PixiuSession|null{
  ...
  try{
    ...
    if(typeof session.username!=='string'||!session.username.trim()||typeof session.token!=='string'||!session.token.trim())return null;
    return {username:session.username,token:session.token};
  }catch{return null}
}

export function clearSession(){localStorage.removeItem('pixiu_session')}
