...
describe('Pixiu Admin API contract',()=>{
  ...
  it('uses the shared contract for runtime resource requests',async()=>{
    const fetchMock=vi.fn().mockResolvedValue(new Response(JSON.stringify({code:'10001',data:[]}),{status:200}))
    vi.stubGlobal('fetch',fetchMock)

    await resourceApi.list()

    expect(fetchMock).toHaveBeenCalledWith(pixiuAdminApi.resources.list,expect.objectContaining({headers:expect.any(Headers)}))
  })

  it('surfaces malformed list responses instead of treating them as empty',async()=>{
    vi.stubGlobal('fetch',vi.fn().mockResolvedValue(new Response(JSON.stringify({code:'10001',data:'not-an-array'}),{status:200})))
  ...
  it('does not accept an error HTTP status as a successful API response',async()=>{
    vi.stubGlobal('fetch',vi.fn().mockResolvedValue(new Response(JSON.stringify({message:'backend unavailable'}),{status:503})))
